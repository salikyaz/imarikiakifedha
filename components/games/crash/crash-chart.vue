    ...
    <div ref="crashWrapper" class="crash__wrapper">
      ...
      <div ref="crashChartContainer" class="crash__chart">
        <div v-show="timeleft > 0 && isActive" class="crash__countdown">
          <span class="w-full text-center crash__start-at">Starting in</span>
...
export default {
  ...
  methods: {
    ...
    startCountDown() {
      if (!this.isActive) {
        this.timeleft = 1000 // Set the initial value to 5000 milliseconds (5 seconds)
      }

      const countdownInterval = 50 // Set a smaller interval (e.g., 50 milliseconds)

      this.interval = setInterval(() => {
        if (this.timeleft <= 0) {
          return clearInterval(this.interval)
        }
        this.timeleft -= countdownInterval // Decrease timeleft by the smaller interval
      }, countdownInterval)
    },
    handleResize() {
      this.loaded = false
